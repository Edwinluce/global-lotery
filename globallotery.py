...
def api_admin_ganancias():
    ...
    rows=c.fetchall(); con.close()
    return jsonify([{"id":r[0],"fecha":r[1][:16] if r[1] else "","recaudado":int(float(r[2] or 0)),"tu25":int(float(r[3] or 0)),"pago75":int(float(r[4] or 0)),"estado":r[5]} for r in rows])
...
def api_admin_apuestas_actual():
    ...
    s=c.fetchone()
    if not s: con.close(); return jsonify({"lista":[],"por_animal":[],"total":0,"sorteo_id":0})
    sid=s[0]
